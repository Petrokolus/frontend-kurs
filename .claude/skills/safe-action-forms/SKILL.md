...
Note: `useStateAction` requires the server action to be defined with `.stateAction()` instead of `.action()`. See the [hooks skill](../../../apm_modules/next-safe-action/skills/skills/safe-action-hooks/use-state-action.md) for the full decision table on when to use `useAction` vs `useStateAction`.
