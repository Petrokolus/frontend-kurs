...
#### Oppgave 6c – Bygg `SkjemaFelt` med `useFormContext`

Nå skal vi begynne å bygge selve skjemaet. Første steg er å gjøre `SkjemaFelt`-komponenten fra oppgave 5 om til å lese `form` fra context i stedet for å ta den inn som prop.

Vi kan samtidig forenkle props-typen: i stedet for å sende inn feilmeldingsteksten som en streng, sender vi bare `required?: boolean` og konstruerer feilmeldingen fra `label`-proppen.
...
#### Oppgave 6d – Bygg `RedigerSpillerSkjema` med forhåndsutfylte verdier
...
Rating redigeres ikke manuelt, den regnes ut automatisk av serveren etter hver registrerte kamp basert på et ELO-system (se forklaringen i Oppgave 3 hvis du er nysgjerrig på detaljene). `rating` er derfor bevisst utelatt fra `SkjemaData` siden det ikke er et felt brukeren skal kunne endre.

</details>

1. En `SkjemaData`-type, identisk med den i `opprett-spiller-skjema.tsx`
2. En `RedigerSpillerSkjema`-komponent som tar imot `spiller: Spiller` som prop, kaller `useForm` med `defaultValues`, og wrapper innholdet i `FormProvider`
...
`FormProvider` wrapper skjemaet og gjør `form`-objektet tilgjengelig for alle komponenter inni, inkludert `SkjemaFelt` fra forrige oppgave. Du sprer hele `form`-objektet inn i `FormProvider` med `{...form}`:
...
#### Oppgave 6e – Utforsk PUT-endepunktet i API-dokumentasjonen
...
<details class="losningsforslag">
<summary>Løsningsforslag 6f</summary>
